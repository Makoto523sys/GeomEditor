...
test('construction node picking works without CAD and returns exact coordinates',()=>{const v=viewer();v.mode='node';v.data.nodes=[{id:'N1',point:[0,20,10]}];let hit;v.onPick=h=>hit=h;v.pick(340,180,true);assert.equal(hit.id,'N1');assert.deepEqual(hit.point,[0,20,10]);assert.equal(hit.exact,true)});
test('construction line picking is separate from CAD edges',()=>{const v=viewer();v.mode='line';v.data.lines=[{id:'L1',points:[[0,-20,0],[0,20,0]]}];let hit;v.onPick=h=>hit=h;v.pick(300,200,false);assert.equal(hit.id,'L1');v.mode='edge';v.pick(300,200,false);assert.equal(hit,null)});
test('construction node picking has finite screen tolerance',()=>{const v=viewer();v.mode='node';v.data.nodes=[{id:'N1',point:[0,20,10]}];let hit;v.onPick=h=>hit=h;v.pick(100,100,false);assert.equal(hit,null)});
